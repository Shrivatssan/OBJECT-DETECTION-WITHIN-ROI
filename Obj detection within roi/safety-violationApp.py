...
class AppROI:
    ...
    def __init__(self, VIDEOPATH, CAMERA, START=True):
        if CAMERA == True:
            self.video = cv2.VideoCapture('videos/scene3.mpg')
        else:
    ...
    def roiMask(self, *src):
        ...
        modiframe = cv2.bitwise_or(fg_zero, bg)
    ...
    def main(self):
        ...
        while (self.video.isOpened()):
            ...
            diff = cv2.absdiff(self.flag, self.frame)
            ...
            for i in range(detections.shape[2]):
                ...
                if confidence > THRESHOLD:
                    ...
                    if self.check_location(xmin, xmax, ymax) == True:
                        ...
                        cv2.rectangle(self.frame_modified, (xmin, yminlabel - labelSize[1]),
                                      (xmin + labelSize[0], ymin + baseLine), RED, cv2.FILLED)
                        cv2.putText(self.frame_modified, label, (xmin, yminlabel), cv2.FONT_HERSHEY_SIMPLEX, 0.3, WHITE,
                                    1, cv2.LINE_AA)
                        # try:
                        #     sound.play()
                        #
